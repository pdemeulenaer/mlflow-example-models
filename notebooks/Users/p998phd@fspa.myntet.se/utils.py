...
def detrend(x, trend):
    ...
    :return (list): Detrended timeseries
    '''
    detrend = np.around(np.array(x) - np.array(trend), decimals=3)
...
def retrend(x, trend):
    ...
    '''
    retrend = np.around(np.array(x) + np.array(trend), decimals=3)
...
def visualization_prediction(df, start_date, end_date, N_days_X, N_days_y, R2_array_1month, R2_array_3month, choice_AIlab_or_local, serving=False):
    '''
    ...
    #R2_all_3month, R2_array_3month, R2_all_1month, R2_array_1month = metric_extraction(y_test_ready, y_pred, N_days_y)
    ...
    if choice_AIlab_or_local == 1:
        # Saving the X,y parts of train, val, test sets in csv
        # in distributed mode for later plots

        from hops import hdfs

        balance_pd.to_csv('balance_pd1.csv', index=None)
        balance_y_pd.to_csv('balance_y_pd1.csv', index=None)
        X_test_pd.to_csv('X_test_pd1.csv', index=None)
        if not serving:
            y_test_pd.to_csv('y_test_pd1.csv', index=None)
        y_pred_pd.to_csv('y_pred_pd1.csv', index=None)
        y_pred_f_pd.to_csv('y_pred_f_pd1.csv', index=None)

        hdfs.copy_to_hdfs("balance_pd1.csv", hdfs.project_path() + "Logs", overwrite=True)  # copy from local fs to hdfs
        hdfs.copy_to_hdfs("balance_y_pd1.csv", hdfs.project_path() + "Logs",
                          overwrite=True)  # copy from local fs to hdfs
        hdfs.copy_to_hdfs("X_test_pd1.csv", hdfs.project_path() + "Logs", overwrite=True)  # copy from local fs to hdfs
        if not serving:
            hdfs.copy_to_hdfs("y_test_pd1.csv", hdfs.project_path() + "Logs",
                                          overwrite=True)  # copy from local fs to hdfs
        hdfs.copy_to_hdfs("y_pred_pd1.csv", hdfs.project_path() + "Logs", overwrite=True)  # copy from local fs to hdfs
        hdfs.copy_to_hdfs("y_pred_f_pd1.csv", hdfs.project_path() + "Logs",
                          overwrite=True)  # copy from local fs to hdfs
    ...
    if not serving:
        ...
        plt.show()
        fig.savefig('performance_R2.png')

        if choice_AIlab_or_local == 4:
            display(fig)

        if choice_AIlab_or_local == 1:
            hdfs.copy_to_hdfs("performance.png", hdfs.project_path() + "Logs",
                              overwrite=True)  # copy from local fs to hdfs
            hdfs.copy_to_hdfs("performance_R2.png", hdfs.project_path() + "Logs",
                              overwrite=True)  # copy from local fs to hdfs
